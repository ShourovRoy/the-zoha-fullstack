...
import { AlertCircle, ShieldAlert, ArrowRight, ShoppingCart, Calendar } from "lucide-react"
...
const CheckoutCancelPage = async ({ searchParams }: PageProps) => {
      ...
      <div className="w-full max-w-md bg-white border border-stone-200/80 rounded-2xl p-6 shadow-sm space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <div className="space-y-0.5">
            <h1 className="text-stone-900 text-base font-bold tracking-tight">Payment Cancelled</h1>
            <p className="text-xs text-stone-400">You abandoned the checkout session prior to authorization.</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between gap-2 bg-stone-50/80 border border-stone-100 p-2.5 rounded-xl text-[11px] font-medium text-stone-500">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="capitalize text-amber-600 font-semibold">{orderDetails.orderProcessStatus || "Cancelled"}</span>
          </div>
        </div>

        {/* Ledger Breakdown Receipt */}
        <div className="space-y-3">
          <h3 className="text-xs font-bold text-stone-800 tracking-wider uppercase">Pending Invoice Ledger</h3>
          <div className="border border-stone-100 rounded-xl divide-y divide-stone-100 overflow-hidden text-xs">
            ...
            <div className="flex justify-between p-3">
              <span className="text-stone-400">Items Retained</span>
              <span className="font-medium text-stone-700">{orderDetails.totalOrderItems || 0} units</span>
            </div>
        ...
        {/* Help message block explaining checkout recovery */}
        <p className="text-[11px] text-center text-stone-500 bg-stone-50/60 py-3 px-4 rounded-xl border border-stone-200/50 leading-relaxed">
          Don&apos;t worry, your items haven&apos;t been cleared. Your persistent checkout context remains saved if you want to complete payment later.
        </p>
