import { getAllCompletedOrders } from "@/lib/data/order-data"
import { ShoppingBag, Calendar, User, Phone, MapPin, Eye, Package, AlertCircle } from "lucide-react"
import Link from "next/link"

const CompletedOrdersPage = async () => {
    const { completedOrders, errorMessage } = await getAllCompletedOrders()
        ...
        <div className="min-h-screen bg-stone-50/50 p-4 sm:p-6 lg:p-8 space-y-6">
            ...
            {errorMessage ? (
                <div className="text-center py-12 border border-red-200 bg-red-50/30 rounded-2xl max-w-md mx-auto space-y-3 p-6">
                    ...
                    <h3 className="text-sm font-bold text-red-800">Unable to load orders</h3>
                    <p className="text-xs text-red-600/80 max-w-60 mx-auto leading-relaxed">{errorMessage}</p>
                    <div className="pt-2">
                        <button
                            onClick={undefined} // Server actions or window.location.reload() if Client Component
                            className="text-xs font-semibold text-stone-700 bg-white border border-stone-200 hover:bg-stone-50 rounded-xl px-4 py-2 transition-colors shadow-2xs"
                        >
                            Try Refreshing
                        </button>
                    </div>
                ...
                (!completedOrders || completedOrders.length === 0) ? (
                    ...
                    /* 3. ACTIVE CONTENT ORDERS GRID LAYOUT */
