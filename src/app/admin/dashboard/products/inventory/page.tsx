...
const InventoryPage = async () => {
    ...
    if (!products || products.length === 0) {
        return (
            <div className="max-w-4xl mx-auto p-8 mt-12 text-center border-2 border-dashed border-neutral-200 rounded-2xl bg-stone-50/40">
                <AlertCircle className="h-10 w-10 text-neutral-400 mx-auto mb-3" />
                <h3 className="text-sm font-semibold text-neutral-900">No Inventory Records Found</h3>
                <p className="text-xs text-neutral-500 mt-1">Get started by creating your first product listing entry.</p>
                <Link
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-neutral-50/70 border-b border-neutral-200 text-neutral-600 text-xs font-semibold uppercase tracking-wider">
                                ...
                                <th className="p-4 hidden sm:table-cell text-center">Gallery</th>
                            ...
                            {products.map((product) => {
                                ...
                                return (
                                    <tr key={product.id} className="hover:bg-neutral-50/50 transition-colors">
                                        ...
                                        {/* Gallery Asset Count Column */}
                                        <td className="p-4 hidden sm:table-cell text-center">
                                            <div className="inline-flex items-center gap-1 px-2 py-0.5 bg-amber-50 border border-amber-200/50 rounded-md text-amber-800 text-xs font-medium">
                                                <Layers className="h-3 w-3 text-amber-500" />
                                                <span>{product.galleryImages?.length || 0}</span>
                                            </div>
                                        </td>

                                        {/* Row Operational Controls */}
                                        <td className="p-4">
